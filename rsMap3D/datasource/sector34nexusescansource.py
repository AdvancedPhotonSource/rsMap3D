...
class Sector34NexusEscanSource(object):
    '''
    '''
    ...
    def loadSource(self):
        ...
        if self.files == None:
            fileFilter = str(os.path.join(self.projectDir, self.projectName)) + \
                         "_[0-9]*" + \
    ...
    def qsForDetector(self):            
        ...
        for row in yIndexArray:
            for column in xIndexArray:
                ...
                qForPixel = self.pixel2q(row, column,None)
    ...
    def pixel2XYZ(self, pixelX, pixelY):
        ...
        xyz = [(self.rho[0][0]*xp + self.rho[0][1]*yp + self.rho[0][2]*zp), \
               ...
               (self.rho[1][0]*xp + self.rho[2][1]*yp + self.rho[2][2]*zp)]
        return xyz
    ...
    def pixel2q(self, pixelX, pixelY, qhat, depth=0):
        ...
        if koutnorm != 0:
            kout /= koutnorm 
        #print "kout, ki %s, %s" % (str(kout), str(ki))
        dp = np.cross(kout, ki)
        #print dp
        #theta = np.arccos(dp)/2
